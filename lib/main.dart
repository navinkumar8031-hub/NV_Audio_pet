      ...
      body: SafeArea(
        child: Stack(
          children: [
                  ...
                  child: Center(
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          const SizedBox(height: 70), // Headroom for Pet Swings
            ...
            // --- 2. GLOBAL NEON-LASER SWINGING CYBER PET OVERLAY ---
            Positioned.fill(
              child: IgnorePointer(
                ignoring: false,
...
// =========================================================================
// 4. CYBER PET SHIMEJI ENGINE WITH DYNAMIC LASER GRAPPLE SWING
// =========================================================================
...
class _CyberPetLaserOverseerState extends State<CyberPetLaserOverseer> with TickerProviderStateMixin {
  ...
  Offset _currentPos = const Offset(0.0, -180.0);
  Offset _startPos = const Offset(0.0, -180.0);
  Offset _targetPos = const Offset(0.0, -180.0);
  Offset? _laserAnchor; // Target ledge where laser beam connects
  ...
  // Smart Anchor Points (Master Top, Subwoofer Roof, Sleep Timer, Edge)
  List<Offset> _getAnchors(Size s) {
    return [
      const Offset(0.0, -180.0),             // Master Top Center
      const Offset(-75.0, -180.0),           // Master Top Left
      const Offset(75.0, -180.0),            // Master Top Right
      const Offset(-20.0, -80.0),            // Sitting on Master Edge
      Offset(-s.width * 0.24, 75.0),         // Subwoofer Card
      Offset(s.width * 0.24, 75.0),          // Sleep Timer Card
    ];
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _planNextUnpredictableMove();
    });
  }

  void _planNextUnpredictableMove() {
    if (!mounted) return;
    final size = MediaQuery.of(context).size;
    final anchors = _getAnchors(size);

    final nextTarget = anchors[_rng.nextInt(anchors.length)];
    final double dist = (nextTarget - _currentPos).distance;

    _startPos = _currentPos;
    _targetPos = nextTarget;
    _facingDirection = (nextTarget.dx >= _currentPos.dx) ? 1.0 : -1.0;

    // Laser beam anchor point placed slightly above target card
    _laserAnchor = Offset(
      (nextTarget.dx + _startPos.dx) / 2 + (_rng.nextBool() ? 35 : -35),
      min(_startPos.dy, nextTarget.dy) - 90,
    );

    // 1. Trigger Shoot Pose first (Charging neon laser)
    setState(() => _currentPose = PetPose.shoot);

    Future.delayed(const Duration(milliseconds: 320), () {
      if (!mounted) return;
      // 2. Mid-air Laser Swing
      setState(() => _currentPose = PetPose.swing);
      final int moveMs = (1400 + (dist * 5.5)).toInt().clamp(1200, 2600);
  ...
  void _updateSwingKinematics() {
    ...
    // Cubic Bézier arc path for pendulum swinging gravity
    final Offset p0 = _startPos;
    final Offset p1 = Offset(_laserAnchor!.dx, _laserAnchor!.dy + 50);
  ...
  void _onArrivedSuperheroLanding() {
    _laserAnchor = null; // Disengage laser line

    // 3. Trigger 3-Point Superhero Landing Pose
    setState(() => _currentPose = PetPose.land);

    Future.delayed(const Duration(milliseconds: 450), () {
      if (!mounted) return;

      // Settle down based on location
      if (_currentPos.dy > 60.0 && _currentPos.dx < 0) {
        // At Subwoofer Card
        setState(() => _currentPose = PetPose.idle);
        widget.onTamperSubwoofer();
      } else if (_currentPos.dy > -110.0 && _currentPos.dy < 0) {
        // Sitting on Master Gain ledge
      ...
      // Plan next jump in 3 to 6 seconds
      _behaviorTimer?.cancel();
      _behaviorTimer = Timer(Duration(milliseconds: 3000 + _rng.nextInt(3000)), () {
        if (mounted) _planNextUnpredictableMove();
      });
    });
  }

  void _startMiniPatrol() {
    // Alternate walk / idle on card ledge
    Timer.periodic(const Duration(milliseconds: 350), (timer) {
      if (!mounted || _currentPose != PetPose.idle && _currentPose != PetPose.walk) {
        timer.cancel();
        return;
      }
      _walkAlternate = !_walkAlternate;
      setState(() => _currentPose = _walkAlternate ? PetPose.walk : PetPose.idle);
    });
  }

  void _onPokePet() {
    // Tap reaction: Immediately fire laser and escape!
    _behaviorTimer?.cancel();
    _swingCtrl.stop();
    _planNextUnpredictableMove();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: Stack(
        ...
        children: [
          // 1. Real-time Glowing Laser Beam Canvas
          if (_laserAnchor != null && (_currentPose == PetPose.shoot || _currentPose == PetPose.swing))
            CustomPaint(
              size: const Size(double.infinity, double.infinity),
              painter: _NeonLaserGrapplePainter(
                handOrigin: _currentPos,
          ...
          // 2. Character Sprite With Interactive Physics
          Transform.translate(
            offset: _currentPos,
              ...
              child: GestureDetector(
                ...
                child: SizedBox(
                  ...
                  child: Image.asset(
                    ...
                    fit: BoxFit.contain,
                    // If image is transparent PNG, it renders seamlessly
...
// =========================================================================
// 5. NEON LASER BEAM & ENERGY SPARK PAINTER
// =========================================================================
class _NeonLaserGrapplePainter extends CustomPainter {
  final Offset handOrigin;
  final Offset targetAnchor;
  final Color laserColor;
  final double pulse;

  _NeonLaserGrapplePainter({
    required this.handOrigin,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final pHand = center + handOrigin + const Offset(10, -10);
    final pTarget = center + targetAnchor;

    // 1. Wide Outer Glow
    final glowPaint = Paint()
      ..color = laserColor.withOpacity(0.45)
      ..strokeWidth = 6.0 + sin(pulse * pi) * 2.0
      ..strokeCap = StrokeCap.round
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 5);
    canvas.drawLine(pHand, pTarget, glowPaint);

    // 2. Core Bright Laser Beam
    ...
    // 3. Impact Energy Spark at Target Anchor
    final sparkPaint = Paint()
      ..color = laserColor
      ..maskFilter = const MaskFilter.blur(BlurStyle.solid, 4);
    canvas.drawCircle(pTarget, 5.0 + sin(pulse * pi * 2) * 2.0, sparkPaint);
    canvas.drawCircle(pTarget, 2.5, Paint()..color = Colors.white);
  }

  @override
  bool shouldRepaint(covariant _NeonLaserGrapplePainter oldDelegate) {
    return oldDelegate.handOrigin != handOrigin ||
...
// ==========================================================
// 6. INDIVIDUAL POP-ZOOM VERTICAL FADER
// ==========================================================
...
// ==========================================
// 7. IN-PLACE POP-ZOOM HORIZONTAL SLIDER
// ==========================================
...
// ==========================================
// 8. REAL-TIME EQUALIZER CURVE PAINTER
// ==========================================
